...
## 📝 대시보드 작업 진행 상황 기록 템플릿

> 각 탭별로 진행 상황을 아래와 같이 기록하며 관리하세요.

```
### [데이터 입력 탭]
- [x] 시트/헤더/모달 기본 구조 생성
- [ ] 데이터 입력/저장/불러오기 기능 구현
- [ ] 시트 자동채움/수정 기능 구현
- [ ] 업로드/다운로드 연동
...
1. 데이터셋 슬라이드 개선
   - 그랩으로 슬라이드 기능 구현
   - 스크롤바 숨김 처리 (CSS로 직접 제어)
   - 파일 다운로드/업로드 버튼 높이 h-8로 수정
   - 버튼 스타일 BUTTON_THEME.secondary 적용

2. 데이터셋 상세 정보 모달 개선
   - 모달 크기: max-w-7xl, max-h-[90vh]
   - 제목: "데이터셋 상세 정보" (파란색)
   - 데이터셋 이름과 버튼 영역 (불러오기, 삭제)
   - 버튼 스타일: BUTTON_THEME.primary, BUTTON_THEME.danger_fill
   - 테이블 형식으로 데이터 표시
   - 글자 크기 text-xs로 설정
   - 셀 높이 h-6로 설정
   - 헤더 고정 (sticky)
